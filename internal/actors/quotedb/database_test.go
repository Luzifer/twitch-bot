...
func TestQuotesRoundtrip(t *testing.T) {
	...
	require.NoError(t, err, "querying empty database")
	assert.Zero(t, cq, "expecting no quotes")

	for i, q := range quotes {
		require.NoError(t, addQuote(dbc, channel, q), "adding quote %d", i)
	}
	...
	require.NoError(t, delQuote(dbc, channel, 1), "removing one quote")
	require.NoError(t, delQuote(dbc, channel, 1), "removing one quote")
	...
	require.NoError(t, setQuotes(dbc, channel, quotes), "replacing quotes")
	...
	require.NoError(t, err, "getting random quote")
	assert.NotZero(t, idx, "index must not be zero")
	assert.NotEmpty(t, q, "quote must not be zero")

	idx, q, err = getQuote(dbc, channel, 3)
	require.NoError(t, err, "getting specific quote")
